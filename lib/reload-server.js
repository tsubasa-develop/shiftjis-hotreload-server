...
const server = http.createServer(function (req, res) {
  ...
  if (fileEnding === 'html' || pathname === '/' || pathname === '' || noFileEnding) { // Server side inject reload code to html files
    if (pathname === '/' || pathname === '') {
      ...
      else if (fs.existsSync(dir + '/' + pathname + '/index.htm')) {
        pathname = dir + '/' + pathname + '/index.htm'
      }
      else if (fallback) {
        pathname = dir + '/' + startPage
      }
    ...
    fs.readFile(pathname, function (err, contents) {
      if (err) {
        ...
        const buf = Buffer .from(contents, 'binary');
        contents = iconv.decode(buf, "Shift_JIS");
        contents += '\n\n<!-- Inserted by Reload -->\n<script src="/reload/reload.js"></script>\n<!-- End Reload -->\n'
        res.setHeader('Content-Type', 'text/html')
        res.end(iconv.encode(contents, 'Shift_JIS'))
      }
    })
